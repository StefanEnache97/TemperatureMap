...
object Interaction2 extends Interaction2Interface {
  ...
  val color_scale_dev: Seq[(Temperature, Color)] = Seq(
    (7.0, Color(0,0,0)),
    (4.0, Color(255,0,0)),
    (2.0, Color(255, 255, 0)),
  ...
  val color_scale_temp: Seq[(Temperature, Color)] = Seq(
  ...
  def availableLayers: Seq[Layer] = {
    Seq(Layer(LayerName.Temperatures, color_scale_temp, 1975 to 2015),
      Layer(LayerName.Deviations, color_scale_dev, 1991 to 2015))
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Year]): Signal[Year] = {
    ...
    Signal(
      if (sliderValue() <= selectedLayer().bounds.min)
        selectedLayer().bounds.min
      else if (sliderValue() >= selectedLayer().bounds.max)
